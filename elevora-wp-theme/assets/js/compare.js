...
// Remove specifically from compare
...
// Update compare buttons on active screen
...
function renderComparePage() {
  ...
  if (list.length === 0) {
    compareTableContainer.innerHTML = `
      <div class="empty-state-wrap" style="text-align: center; padding: 80px 24px; border: 1px dashed var(--border-color); border-radius: var(--border-radius-lg); background-color: var(--bg-secondary);">
  ...
  // Fetch detailed info for selected products
  const products = list.map(id => ElevoraProducts.find(p => p.id === id)).filter(p => p !== undefined);

  // Extract all unique specs key names across compared products
  ...
  let tableHeaderHtml = `
    <div class="compare-row compare-header-row" style="display: grid; grid-template-columns: 240px repeat(${products.length}, 1fr); gap: 20px; align-items: stretch; border-bottom: 2px solid var(--border-color); padding-bottom: 24px;">
      <div class="compare-cell label-cell" style="display: flex; flex-direction: column; justify-content: flex-end;">
  ...
  products.forEach(p => {
    ...
    tableHeaderHtml += `
      <div class="compare-cell product-header-cell" style="position: relative; display: flex; flex-direction: column; text-align: center; border: 1px solid var(--border-color); border-radius: var(--border-radius-lg); padding: 20px; background-color: var(--bg-secondary);">
        ...
        <div style="height: 120px; overflow: hidden; margin-bottom: 16px; border-radius: var(--border-radius-md); background-color: var(--bg-primary);">
          <img src="${p.images[0]}" alt="${p.name}" style="width: 100%; height: 100%; object-fit: contain;">
        </div>
        <span style="font-size: 0.75rem; font-weight: 700; color: var(--accent-dark); text-transform: uppercase; margin-bottom: 6px; display: block;">${p.category}</span>
  ...
  // Brand Row
  tableBodyHtml += `
    <div class="compare-row" style="display: grid; grid-template-columns: 240px repeat(${products.length}, 1fr); gap: 20px; align-items: center; padding: 12px 0; border-bottom: 1px solid var(--border-color);">
      <div class="compare-cell label-cell" style="font-weight: 700; color: var(--text-primary); font-size: 0.9rem;">Brand</div>
  `;
  products.forEach(p => {
    tableBodyHtml += `<div class="compare-cell val-cell" style="font-size: 0.9rem; text-align: center; font-weight: 600;">${p.brand}</div>`;
  });
  tableBodyHtml += `</div>`;
  ...
  tableBodyHtml += `
    <div class="compare-row" style="display: grid; grid-template-columns: 240px repeat(${products.length}, 1fr); gap: 20px; align-items: center; padding: 12px 0; border-bottom: 1px solid var(--border-color);">
      <div class="compare-cell label-cell" style="font-weight: 700; color: var(--text-primary); font-size: 0.9rem;">Customer Rating</div>
  `;
  products.forEach(p => {
    tableBodyHtml += `
      <div class="compare-cell val-cell" style="font-size: 0.9rem; text-align: center; display: flex; flex-direction: column; align-items: center;">
        <span style="font-weight: 700; margin-bottom: 2px;">${p.rating} / 5.0</span>
        <div style="font-size: 0.75rem; color: var(--accent-dark);">${generateStars(p.rating)}</div>
  ...
  allSpecsKeys.forEach(specKey => {
    tableBodyHtml += `
      <div class="compare-row" style="display: grid; grid-template-columns: 240px repeat(${products.length}, 1fr); gap: 20px; align-items: center; padding: 12px 0; border-bottom: 1px solid var(--border-color);">
        <div class="compare-cell label-cell" style="font-weight: 700; color: var(--text-primary); font-size: 0.9rem;">${specKey}</div>
    `;
    products.forEach(p => {
      const specVal = p.specs && p.specs[specKey] ? p.specs[specKey] : `<span style="color: var(--text-muted);">N/A</span>`;
      tableBodyHtml += `<div class="compare-cell val-cell" style="font-size: 0.85rem; text-align: center; color: var(--text-secondary); line-height: 1.4;">${specVal}</div>`;
    });
    tableBodyHtml += `</div>`;
  });
